    ...
    $output->writeln(
      $this->llm->generate(
        ...
        (new GenerateOptions())
          ->setTemperature(0.8),
        tools: [
          new Tool('getWeather', 'Retrieves current weather', function (WeatherRequest $request) {
            return file_get_contents(
              'https://api.open-meteo.com/v1/forecast?latitude='.$request->latitude.'&longitude='.$request->longitude.'&current=temperature,windspeed'
            );
          })
        ]
      )->message->content
